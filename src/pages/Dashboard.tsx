...
export function Dashboard({ go }: { go: (r: Route) => void }) {
  ...
  const isEmpty = db.investors.length === 0

  /*
   * فرعا الفراغ واللوحة داخل شجرة واحدة، ونافذة الإضافة بعدهما في موضع
   * ثابت. أول مبلغ يُضاف يبدّل الفرع، ولو كانت النافذة داخل أحدهما لأعاد
   * React تركيبها عند التبديل فتضيع حالتها في منتصف الإضافة.
   */
  return (
    <>
      {isEmpty ? (
